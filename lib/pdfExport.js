...
function getToolSlug(toolName) {
  ...
  if (toolName === "Land Use & Capacity Estimator") {
    return "land-use-capacity";
  }
...
function pickExecutiveMetrics(selectedResults, currency) {
  ...
  if (metrics.length < 4 && selectedResults.landuse?.results?.installedKwp !== undefined) {
    const installedKwp = Number(selectedResults.landuse.results.installedKwp);
    metrics.push({
      label: "Installable Capacity",
      value:
        Number.isFinite(installedKwp) && installedKwp >= 1000
          ? `${formatAxisValue(installedKwp / 1000)} MWp`
          : `${formatAxisValue(installedKwp)} kWp`,
    });
  }
