    ...
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 sm:py-16">
      <div className="text-center">
        ...
        <p className="mx-auto mt-3 max-w-xl text-[var(--color-muted)]">
          Have a question, feedback, or partnership opportunity? We would love to hear
          from you.
        </p>
      </div>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-3">
        {infoCards.map((card) => (
          ...
          >
            ...
            <p className="mt-1 text-xs text-[var(--color-muted)]">{card.description}</p>
            {card.href ? (
              <a
                href={card.href}
              ...
              <p className="mt-2 text-sm font-medium text-[var(--color-brand)]">{card.value}</p>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="mt-12 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 sm:p-8">
        <h2 className="text-xl font-semibold text-[var(--color-text)]">Send us a message</h2>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              value={form.message}
              onChange={handleChange}
              className="w-full resize-none rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-3.5 py-2.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] focus:border-[var(--color-brand)] focus:outline-none focus:ring-1 focus:ring-[var(--color-brand)]"
